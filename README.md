...
| Surface | URL | Role |
|---------|-----|------|
| **WebUI** | `http://127.0.0.1:8787` | Web chat frontend |
...
**Architecture:** Browser chat is routed through the gateway (`HERMES_WEBUI_CHAT_BACKEND=gateway`), so tools, memory, cron, and the `agents:local` Docker sandbox stay on the gateway — WebUI is not a second agent. Requires `API_SERVER_ENABLED` + `HERMES_API_SERVER_KEY` (auto-generated by `./start.sh` if empty).
