...
# Bearer key for gateway API server (:8642) — WebUI chat backend uses the same key.
ensure_hermes_api_server_key() {
  ...
  if [[ -z "${HERMES_API_SERVER_KEY:-}" ]]; then
    ...
    {
      printf '\n'
      printf '# Hermes API server key (:8642) — WebUI gateway chat; auto-generated by start.sh\n'
